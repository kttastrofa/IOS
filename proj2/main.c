...
/// what if someone gives incompatible values? PERROR! ~ we do warn the user :) and exit with value 1
        if (!(skierMIN && skierMAX && stopsMAX && stopsMIN && busMIN && busMAX && WskierMIN && WskierMAX && WbusMIN && WbusMAX)){
            perror("Wrong values used, contact the author for more information.\n");
            exit (1);
        }
        //TODO chceck for integers
...
/// initiate shared memory for needed variables as semaphores or integers meant for checking the "stav"
        ...
        if (shmleave < 0) {
            perror("error shmget");
                ...
                sem_destroy(stop);
                shmdt(stop);
                shmctl(shmstop, IPC_RMID, NULL); // taking care of the memory, if failed
            exit(1);
        }
        ...
        if (shmfinal < 0) {
            perror("error shmget");
                ...
                sem_destroy(leave);
                shmdt(leave);
                shmctl(shmleave, IPC_RMID, NULL); // taking care of the memory, if failed
            exit(1);
        }
        ...
        if (shmA < 0) {
            perror("error shmget");
                ...
                sem_destroy(final);
                shmdt(final);
                shmctl(shmfinal, IPC_RMID, NULL); // taking care of the memory, if failed
            exit(1);
        }
        A = shmat(shmA, NULL, 0);
        if (A == (int*) -1) {
        ...
        *A=0; // variable of the number of actions
        ...
        if (shmidZ < 0) {
            perror("error shmget");
                ...
                shmdt(A);
                shmctl(shmA, IPC_RMID, NULL); // taking care of the memory, if failed
            exit(1);
        }
    ...
    int shmARR = shmget(IPC_PRIVATE, sizeof(int), IPC_CREAT | 0666);
        if (shmARR < 0) {
            perror("error shmget");
                ...
                shmdt(idZ);
                shmctl(shmidZ, IPC_RMID, NULL); // taking care of the memory, if failed
            exit(1);
        }
        ...
        if (shmBOARD < 0) {
            perror("error shmget");
                ...
                shmdt(ARR);
                shmctl(shmARR, IPC_RMID, NULL); // taking care of the memory, if failed
            exit(1);
        }
        ...
        *BOARD=0; // variable for counting skiers on board of the bus ~ controlled by constant variable of capacity
        ...
        if (shmwaiting < 0) {
            perror("error shmget");
                ...
                shmdt(BOARD);
                shmctl(shmBOARD, IPC_RMID, NULL); // taking care of the memory, if failed
            exit(1);
        }
...
/// here we call functions as processes. "fork" takes care of having separated processes running on the "same" time
    ...
    shmdt(waiting);
    shmctl(shmwaiting, IPC_RMID, NULL); // taking care of the memory once again, to be sure :)
    exit (0);
}
